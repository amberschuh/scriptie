...
gitrootdir <- function() {
  ...
  validationfile <- normalizePath(paste(rootdir,"",sep="/"))
...
datecols <- c("State.of.emergency.issued",
...
polfix <- read.csv("COVID-19 Polic Variables fix - Blad1.csv")
...
jhucoviddata <- function() {
  confirmed = read.csv('time_series_covid19_confirmed_US.csv', stringsAsFactors=FALSE)
  deaths = read.csv('time_series_covid19_deaths_US.csv', stringsAsFactors=FALSE)
...
gmr <- read.csv("Global_Mobility_Report.csv", header=TRUE, stringsAsFactors = FALSE)
...
temp <- read.csv(paste(rootdir,"Facemask_Mar26_April29.csv",sep="/"))
temp1 <- temp[c("state","fips","z.mask","mask_percent")]

# temp <- read.csv(paste(rootdir,"1976-2016-president.csv.txt",sep="/"))
...
save(covidstates, dataguide, file=paste(rootdir,"covidstates.Rda",sep="/"))
write.csv(covidstates, paste(rootdir, "scriptiedata.csv", sep="/"), row.names=FALSE)